...
class ModelConfig:
    # Channel multiplier configuration (adjustable model scale)
    channel_mult = [2, 4, 8, 16, 32]  # Original: [1, 2, 4, 8, 16]
    num_res_blocks = 3  # Number of residual blocks per layer
    time_dim = 512  # Time embedding dimension
    attn_layers = [2, 3]  # Layer indices for attention mechanism
    dropout = 0.1
    use_adaGN = True  # Use adaptive normalization
...
class DiffusionUtils:
    ...
    def sample(self, model, unet_model, features, shape, device, ddim=False, eta=0.0, denoise_step=5):
        ...
        if ddim:
            """Generate uniformly spaced timestep sequence"""
            ...
            for i in tqdm(reversed(range(0, self.ddim_timesteps)), desc='sampling loop time step', total=self.ddim_timesteps):
                _t = _t_list[i]
                t = torch.full((b,), _t, device=device, dtype=torch.long)  # ddim uses skip sampling
                img = self.p_sample_ddim(model, img, features, t, i, eta=eta)
        else:
            for i in tqdm(reversed(range(0, self.timesteps)), desc='sampling loop time step', total=self.timesteps):
                if i < denoise_step:
                    t = torch.full((b,), i, device=device, dtype=torch.long)  # ddpm uses original timesteps
